...
- **The app family's layout** — the same page width and the same header as [Sprint Predictability](https://github.com/eagleadams86/sprint-velocity), [Flow Metrics](https://github.com/eagleadams86/team-dashboard) and [PAPTrack](https://github.com/eagleadams86/paptrack). The header is a bar across the top that **stays put as you scroll**, so the theme picker and the link to the other page are always a click away rather than somewhere above the first card. Its controls are always **one line, beside the name**, the same rule as every app in the family: they stay next to the name wherever they fit there, and in a window too narrow for them they scroll sideways — with **‹ ›** arrows at the end of the row for a mouse or trackpad — rather than wrapping onto a second line. With two controls that only happens in a very narrow window. On a phone held upright the name needs the whole width, so the controls sit on a line under it, at the right-hand end. Tabbing onto a control that is half past the edge scrolls it fully into view
...
- **Keyboard and screen-reader landmarks** — a skip link straight to the content, and real `main` and `footer` regions to jump between. The skip link, and every control you reach with Tab or Shift+Tab, lands just below the header bar whatever height the bar is at that moment — on a phone held upright the bar is two lines of name and a line of controls. Until 2026-09-14 the skip link's target landed under the bar there, and Shift+Tab back up either page left controls hidden behind it at every window size
- **Share what's on screen** — the `↗ Share` button in the header turns the figures into a link, with the numbers carried *inside the link itself*: nothing is uploaded and no copy is kept, so there is also no way to withdraw one once sent. The window is the same one [Sprint Predictability](https://github.com/eagleadams86/sprint-velocity), [Flow Metrics](https://github.com/eagleadams86/team-dashboard), Money Map and Golf Handicap open — same width, same panel, same *Copy link / Preview it / Done* — with one difference stated in it: these links are **not** read-only, because a lottery scenario is something you want the other person to play with. Opening someone else's link is deliberately **a look, not a save** — the page says at the top where the figures came from, saves none of them for the whole visit, and *Back to mine* returns you to your own settings exactly as you left them. Pasting a link into a tab already showing the page works too, which is the way most people will actually open one
  - **And the live jackpot does not quietly replace what was sent to you (fixed 2026-09-12).** The two jackpot cards still refresh — they are how you step out of someone else's figures and into today's — but the amount boxes stay open and hold exactly what arrived — the M/B toggles and the answer with them — however long the feed takes to come back. Until this was fixed, a link sent with an advertised jackpot of $750M could open showing the current $232M instead, under a banner still saying the figures came from the sender. Press a game card and you *do* get that game's live figures, because that is you asking for them — and the banner changes to say so, rather than going on vouching for numbers nobody sent
...
Targeted at **Niagara County, NY residents** who want to model what a lottery lump-sum would actually look like as a managed investment portfolio — outside NYC and Yonkers is the default, and both are picks on the residence control since 2026-08-22.
...
- **The hover panel wears the theme** — the theme's own card shade, text and border rather than the chart library's built-in black box, so it belongs to the page on Light and Sepia as much as on Midnight. The same six values Flow Metrics, Sprint Predictability and Money Map use
- **Hover a chart and it names what is under the pointer.** On the two line charts — the 10-year growth projection and the 40-year drawdown — hovering anywhere in a year's column reads out *every* line for that year, rather than only answering when the pointer lands exactly on a point. That matters most on the drawdown, whose three lines are drawn with no points at all: before this, its tooltip could only be summoned by hitting something invisible. The bars and the donut keep the default, because a bar and an arc are their own targets. Same rule, and the same reason, as in Money Map
- **Any chart fills the window** — each of the four carries a ⤢ button in its top-right corner; press it and that chart alone fills the screen under the header. **The header stays where it is and stays usable**: change the theme and the chart redraws in front of you, still full screen. Escape, the same button (now an arrows-in icon), or a click on the margin round the card brings it back down, and the page is where you left it. **Beside the ⤢ are a ‹ and a ›** that step through the four charts without coming back down, wrapping round at both ends; the left and right arrow keys do the same, unless the caret is in a control, where an arrow belongs to that control. A chart in a folded section is not on the walk — unfold it and it is back. It is the same feature, and the same behaviour, as in Flow Metrics, Sprint Predictability and Money Map
...
- **Lump sum, annuity, or the two side by side** — the page answered only the lump-sum half of the actual decision until 2026-08-22. The annuity is modelled properly: 30 payments each 5% larger than the last for Powerball and Mega Millions, 26 equal ones for NY Lotto, with the payment-by-payment schedule and the tax on each year. **Compare** puts them together by following **both payouts invested** — the lump sum all on day one, each annuity payment the year it arrives — at a return you choose, and names the return at which the two finish level: below it the annuity wins, above it the lump sum does. The annuity is often ahead on tax alone, because thirty tax returns each start at the bottom of the brackets where one does not
- **Tax on the growth, when and how it is really taxed** *(2026-09-13)* — Compare used to shrink the annuity to today's money at an after-tax "discount rate", which left the tax on thirty years of investment growth to the reader's imagination. It now runs both payouts forward a year at a time. In **stock index funds** a 1.4% dividend is taxed each year (long-term capital-gains rate, the 3.8% net investment income tax, New York as ordinary income) and the rest of the growth is taxed when the shares are sold after the last payment, against a tracked cost basis; in **bonds or savings** all of it is taxed every year as ordinary income. Each year's investment income goes on the same tax return as that year's annuity payment, so it meets the rate the payment has already pushed you to. **This moved the answer**: tax on growth falls hardest on the lump sum, which has the most invested for longest, so on a $251M Powerball the break-even went from 5.65% to about 6.8% in stock funds — and past 11% in bonds
- **Today's Money or In 30 Years** — a switch on Compare shows the same run from either end: what each payout finishes at after every tax, or what that is worth today (the sum you'd have to invest now to finish in the same place). The two views always agree on the winner. The four tiles follow the switch too, and are all the same kind of money — they used to put the annuity's raw thirty-year total beside figures in today's money, and the biggest number on screen was the one counted differently
- **Invested Year by Year** — on Compare, a table of both balances at the end of each year with that year's tax on the growth, then the tax at the sale and what is left
...
- **Collapsible tax tables** — the two breakdown tables sit in a section you can collapse, and closing it sticks across visits (it opens expanded by default — see below)
- **The investment return and the holding appear on Compare and nowhere else** — they decide which payout is ahead, and Compare is the only view that invests anything. (They replaced a discount rate, which used to show on the Annuity tab too, where the tiles and the schedule are all uninvested, so typing in it changed nothing you could see.)
- **On Compare the working follows the answer** — the effective tax rate, the whole tax breakdown and the payment schedule describe whichever payout is **Ahead here** (the schedule simply isn't there when the lump sum wins — thirty rows of annuity payments under a verdict recommending the lump sum was the page arguing with itself), and change over with it the moment the investment return crosses the break-even. Both say which payout they are about, because the two cards each carry their own rate. They used to be the lump sum's always, so on the tab whose job is to pick a winner the arithmetic underneath could belong to the one that lost. *(One breakdown, not two side by side: the lump sum's describes the whole prize and the annuity's describes one of thirty payments, so in adjacent columns they would invite comparing $300M against $7.5M. The comparable figures — after tax, and in today's money — are already on the cards.)*
- **Help where the page assumes knowledge** — a small circled **i** beside the investment return, the holding, the effective tax rate and each of the two tax stages opens a plain-English explanation, the same affordance every app in the family carries. *(It drew a **?** until 2026-08-23, as did Golf Handicap, while three other apps drew an "i" — one control with two faces. "i" won: "?" is the glyph a browser already puts on its own help cursor and in a form's validation bubble, and it asks a question where this thing answers one.)* The rate one is the reason it exists: the page recommends a payout on the strength of that number, and nothing on screen said what it was. It explains which way the return pushes the answer, and that you don't have to pick the right one — Compare names the return at which the two payouts are level, so all you decide is which side of it you're on
- **Every section opens expanded** — the tax breakdown, the winning numbers, the payment schedule and Invested Year by Year. The schedule was collapsed until 2026-08-22, which made the thirty figures an annuity actually consists of something you had to know to ask for. Close any of them and it stays closed on your next visit
- **Remembers your settings** — game selection, winner count, payout choice, filing status, residence, investment return, how it is invested, which Compare view, and which sections you've collapsed, all restored on revisit
...
figures inside the URL fragment and touch no network at all — and a cached feed answering a
moment later never overwrites them, which is what a warm cache used to do every time.
```
...
argument between them; the other half is that a lump sum is invested for longer, which is
what Compare's investment return is for — both payouts are followed invested, with the tax
on that growth charged in the year it falls due. Future payments are taxed on **today's** brackets — nobody
...
`tests.html` loads both real pages in hidden same-origin iframes and calls their functions directly — no build step, no copies of the code under test. It pins the shared tax engine (the bracket tables, New York's recapture, the withholding floor, the annuity schedules, and the full-year tax that puts investment income on the same return as a lottery payment), the calculator's manual-amount parser, the six games' winning-numbers allowlist, and the winner-count clamp, plus the portfolio's allocation model (`computeIncome`), its forty-year drawdown, the Treasury feed parser, and the shared formatters. Both pages' share links are tested against deliberately hostile payloads: a hand-edited link cannot put a value anywhere it does not belong. Three more boot the calculator in its own frame with a **warm jackpot cache and a share fragment**, which is the setup that used to lose the sender's figures — they pin that the feed leaves the shared amounts alone, that pressing a game card replaces them *and says so*, and that a link carrying no amounts is filled by the feed as usual.
