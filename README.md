...
- **Copy As CSV, and print** — every figure on screen as two clipboard-ready columns (the annuity's full payment schedule included, and the portfolio's forty-year path), and a print stylesheet that drops the header, the buttons and the chrome, opens every collapsed section and puts the two columns into one
...
- **On Compare the effective tax rate says whose it is** — that line, and the tax breakdown under it, describe the **lump sum**; each of the two cards carries its own payout's effective rate. Unqualified it read as the answer for the comparison rather than for one side of it
