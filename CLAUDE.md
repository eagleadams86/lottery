...
- **Both pages wear the app family's chrome since 2026-08-21**, and the pieces of it are the same ones every sibling carries. What to know before touching any of it:
  ...
  - **The header's controls are ONE LINE THAT SCROLLS SIDEWAYS, BESIDE THE NAME, at every width (2026-09-14, family-wide).** Charles: *"should we just make them both always single line side scrollers?"* — Money Map was built first (financial-plan `b14f4ae`) and these pages take its header half; neither has a tab bar. Each page wraps `.top-actions` (still the scroller's name) in a `.headrow` with a `.rownav` of two arrows, and **carries its own verbatim copy of `wireScrollRow(row, nav)`** — they are separate single-file pages with no shared script — called straight after the theme restore. The same evening Charles asked to *"keep buttons beside the name"* (Money Map `2c19e01`): the row takes what the name leaves (`flex: 1 1 0%`) with `justify-content: flex-end` on the ROW, and wraps under the name only when too little is left. What to keep:
    - **The row's floor is NOT Money Map's `min-width: min(15rem, 100%)`, deliberately.** Money Map's controls are wider than 15rem; these two are 195px (209 on touch). A bare 15rem floor wrapped them under the name in every window from 620 to 670px (Chromium; 610–670 in WebKit), where they had always fitted beside it — the opposite of what was asked. The floor is the smaller of 15rem and the controls' own width, written `width: 15rem; max-width: 100%` with the default `min-width: auto`: a flex item whose basis is not `auto` gets an automatic minimum of the lesser of its specified width and its content's minimum, capped by `max-width`. Probed in Chromium and WebKit at 5px steps from 150 to 700px: it wraps at exactly the old width in both, and never scrolls the page sideways. Don't "tidy" it back to the family's line.
    - **No `justify-content: flex-end` on `.top-actions` any more.** In a scroller that overflows it pushes the overflow off the LEFT edge, where no scroll can reach it. On `.headrow` it is safe, because the row never overflows (the scroller inside shrinks). A consequence to know: where the row is on a line of its own (480px, an upright phone) its controls now sit at the RIGHT end of that line — they sat at the left before, as they do in no sibling.
    - **The keyboard reveal is in `wireScrollRow`** (Money Map's `focusin` listener, byte-identical across the family): a control half past the row's edge, focused from the keyboard, is scrolled wholly into the row. The browser only scrolls a focus target that is entirely hidden. It cannot bring in a control WIDER than the visible row — at 170px the theme picker keeps its right edge and loses 22.6px on the left. That is a family-wide limit of the shared listener, left alone rather than forked here.
    - **NOTHING THE PAGE SCROLLS TO LANDS UNDER THE STICKY BAR — the clearance is on the ROOT, and MEASURED.** Both pages: `html { scroll-padding-top: calc(var(--header-h, 96px) + 8px) }`, with `trackHeaderHeight()` (byte-identical in PAPTrack) writing `--header-h` from the header's box, rounded up, kept true by a ResizeObserver. The 8px is room for the focus ring. It covers two jobs:
      - **A keyboard focus** (Charles, 2026-09-14: *"fix the sticky header shift+tab issue too"*; Golf Handicap's `8935363`). The browser scrolls a focused control to the top edge of the window, where the sticky bar sits. Walking real Shift+Tab from the page end, controls hidden behind the bar went: **calculator 15 of 134 at 1280×800, 20 on a 390px phone, 5 on a sideways 844×390 phone; portfolio 19 of 138, 33 and 24 — 0 everywhere after.** Tab down from the top hid none before or after.
      - **The skip link**, which lands `<main>` 8px under the bar. `<main>` had its own `scroll-margin-top` — a fixed 96px, shorter than the upright phone's 114px header (the name wraps to two lines), so the skip link landed `<main>` 18px under the bar on a 390px phone and 69px at 170px; then the header + 20px for a few hours. **It is gone on purpose**: an element's scroll-margin and the root's scroll-padding ADD, so keeping both lands the target 28px down. Don't put it back.
      - Tests (floor 188): the skip-link test now reads the root's padding and asserts `<main>` adds no margin; a new one per page focuses a control scrolled above the window, with a plain `focus()`, and requires it below the header. **Its frame is 200px tall on purpose.** Chrome CENTRES a control a script focuses, where a real Tab lines it up with the top edge, so in the first 300px frame the centre was already below the bar and the live check passed on the broken page. In 200px it is red on the previous commit (calculator 88.2px, portfolio 81.0px against a 114px header bottom).
      - The portfolio's older `measureMaxiTop()` measures the same box for the full-screen chart and is left as it was.
    - The arrows show only while the row overflows, are **mouse and trackpad only** (`@media (hover: none), (pointer: coarse)` hides them), and are `tabindex="-1"` + `aria-hidden`. Plain glyph buttons in each page's tokens — the portfolio's short aliases (`--text2`, `--text`), the calculator's pack names — and the portfolio states its own `:focus-visible` for them, because its global focus rule does not name `button`.
    - Both print blocks wrap the row instead of scrolling it.
    - **With two controls the row practically never overflows.** Measured at 1600, 1100, 705 and 480px, an upright 390px phone and a sideways 844×390 phone, for both the first cut and the correction: no arrows anywhere, header heights unchanged (51px wide, 88.5 at 480, 114 on the upright phone, 51 sideways). What the correction moves is where the controls sit when the row is on a line of its own — the right end of it now, not the left. It only overflows below about 220px, which is why the live test in `tests.html` uses a 170px frame (one line, arrows shown, › scrolls and turns ‹ on) beside a 1280px one (no overflow, no arrows, controls on the name's line).
    - **Three more live tests per page came with the correction** (floor raised to 186). (1) The controls sit beside the name at 660px, where they fit with less than 15rem to spare — this CANNOT be red on the first cut, whose content-sized row already stayed beside the name wherever it fitted, so it was proved red against Money Map's bare 15rem floor instead (33.5px below the name's line). (2) A control made to straddle the row's edge and focused lands wholly inside it (red on the first cut — the theme picker 53.6px clipped). Its frame is SIZED FROM MEASUREMENT, 6px short of the controls' width with the scroll bar counted, because the band where the row overflows yet the picker still fits is a few pixels wide and moves with the platform's scroll bar. (3) The skip link from a scrolled 390px frame lands `<main>` below the header, and the clearance follows the header when the frame widens (red on the first cut — main at 96, header bottom 114).
