...
- **Both pages wear the app family's chrome since 2026-08-21**, and the pieces of it are the same ones every sibling carries. What to know before touching any of it:
  ...
  - **The header's controls are ONE LINE THAT SCROLLS SIDEWAYS, at every width (2026-09-14, family-wide).** Charles: *"should we just make them both always single line side scrollers?"* — Money Map was built first (financial-plan `b14f4ae`) and these pages take its header half; neither has a tab bar. Each page wraps `.top-actions` (still the scroller's name) in a `.headrow` with a `.rownav` of two arrows, and **carries its own verbatim copy of `wireScrollRow(row, nav)`** — they are separate single-file pages with no shared script — called straight after the theme restore. What to keep:
    ...
    - **With two controls the row practically never overflows.** Measured against the previous commit at 1600, 1100, 705 and 480px, an upright 390px phone and a sideways 844×390 phone: both pages pixel-identical, no arrows anywhere. It only overflows below about 200px, which is why the live test in `tests.html` uses a 170px frame (one line, arrows shown, › scrolls and turns ‹ on) beside a 1280px one (no overflow, no arrows, controls on the name's line). Confirmed red on the old pages.
    - Found and deliberately NOT changed with it: `main { scroll-margin-top: 96px }` is shorter than the upright phone's 114px header (the brand wraps to two lines there), so on a phone the skip link lands its target about 18px under the bar. That predates this change.
